...
from cal import (all_calendar_events_in_range, describe, describe_relative,
                 events)
from data.menus import autumn_2019_menus as MENUS
from data.menus import autumn_2019_weeks as WEEKS
from flask import Flask
from flask_ask import Ask, statement
from utils import parse_amazon_date_to_range, run_many
...
def free_on_date(day):
    ...
    """
    all_events = all_calendar_events_in_range(calendars.values(), day, day + timedelta(hours=24))

    if len(all_events) > 0:
        summaries = " ".join(describe_relative(all_events, day))
        return statement((f"It looks like you are not free on {day}." f" You have, {summaries}"))

    return statement("I can't see anything in your calendars. Looks like you're free!")
...
def lunch_today():
    ...
    try:
        ...
        if today < max(WEEKS.keys()):
            return statement("<speak>Are you on half term silly sausage?</speak>")
    ...
    return statement(
        (
            f"Here is the menu for {today} this week."
            f" You can have baked potatoe, {main}, or {vegitarian}."
            f" For pudding there is {pudding}."
        )
    )
